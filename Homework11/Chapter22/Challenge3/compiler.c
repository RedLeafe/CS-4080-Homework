...
typedef struct {
  ...
  int localCount;
  int scopeDepth;
  GlobalDeclaration globals[UINT8_COUNT];
  int globalCount;
} Compiler;
...
static void addGlobal(Token name, bool isMutable) {
  ...
  GlobalDeclaration* global = &current->globals[current->globalCount++];
  global->name = name;
  global->isMutable = isMutable;
}

static GlobalDeclaration* resolveGlobal(Token* name) {
  for (int i = current->globalCount - 1; i >= 0; i--) {
    GlobalDeclaration* global = &current->globals[i];
...
static void namedVariable(Token name, bool canAssign) {
  ...
  if (arg != -1) {
    ...
    getOp = OP_GET_GLOBAL;
    setOp = OP_SET_GLOBAL;
    GlobalDeclaration* global = resolveGlobal(&name);
    if (global != NULL) {
