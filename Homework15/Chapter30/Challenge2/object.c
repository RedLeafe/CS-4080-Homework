...
static ObjShortString* allocateShortString(const char* chars, int length,
                                           uint32_t hash) {
  ObjShortString* s = ALLOCATE_OBJ(ObjShortString, OBJ_SHORT_STRING);
  memcpy(s->chars, chars, (size_t)length);
  s->chars[length] = '\0';
  s->hash = hash;
  push(OBJ_VAL((Obj*)s));
  tableSet(&vm.strings, (Obj*)s, NIL_VAL);
  pop();
  return s;
}
...
Obj* takeString(char* chars, int length) {
  ...
  if (length <= 7) {
    ObjShortString* s = allocateShortString(chars, length, hash);
    FREE_ARRAY(char, chars, length + 1);
    return (Obj*)s;
  }

  return (Obj*)allocateString(chars, length, hash);
}
...
Obj* copyString(const char* chars, int length) {
  ...
  if (length <= 7) {
    return (Obj*)allocateShortString(chars, length, hash);
  }

  char* heapChars = ALLOCATE(char, length + 1);
  memcpy(heapChars, chars, length);
  heapChars[length] = '\0';
  return (Obj*)allocateString(heapChars, length, hash);
}
...
void printObject(Value value) {
  switch (OBJ_TYPE(value)) {
    ...
    case OBJ_SHORT_STRING:
      printf("%s", objectStringChars(AS_OBJ(value)));
      break;
  }
}
