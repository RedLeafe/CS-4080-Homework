...
static Entry* findEntry(Entry* entries, int capacity, Obj* key) {
  uint32_t index = objectStringHash(key) & (capacity - 1);
...
Obj* tableFindString(Table* table, const char* chars, int length,
                     uint32_t hash) {
  ...
  for (;;) {
    ...
    } else if (objectStringHash(entry->key) == hash &&
               objectStringEqualsChars(entry->key, chars, length)) {
      return entry->key;
    }
